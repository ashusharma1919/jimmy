...
$pass = $_GET['updateid'];
$editQuery = "SELECT * FROM `services`WHERE id='{$pass}'";
...
if(isset($_POST['submit'])){
    ...
    if(!$count_error){
        ...
        $id = $_POST['id'];
        $query = "UPDATE `services` SET `service`='{$desc}',`image`='{$imagePath}',`icon`='{$osm}'WHERE id='{$id}'";
        $checkQuery = mysqli_query($con,$query);
        if($checkQuery){
            echo "<script>alert('data updated')</script>";
            echo redirect('admin/ser_show.php');
        }else{
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <textarea id="tiny" name="name" class="form-control" id="exampleInputEmail1"
                                    placeholder="Enter description"><?= $row['service'] ?></textarea>

                            </div>
                            <div class="form-group">
                                    <label for="exampleInputEmail1">Icon</label>
                                    <input type="text" class="form-control" id="exampleInputEmail1"
                                        placeholder="Enter icon" name="icons"value="<?php echo @$row['icon'] ?>">
                                </div>
