...
$pass = $_GET['updateid'];
$editQuery = "SELECT * FROM `about`WHERE id='{$pass}'";
...
if(isset($_POST['submit'])){
    ...
    if(!$count_error){
        ...
        $links = $_POST['link'];
        $id = $_POST['id'];
        $query = "UPDATE `about` SET `description`='{$desc}',`image`='{$imagePath}',`link`='{$links}'WHERE id='{$id}'";
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <textarea id="tiny" name="name" class="form-control" id="exampleInputEmail1"
                                    placeholder="Enter description"><?= $row['description'] ?></textarea>

                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail1">Btn link</label>
                                <input type="url" class="form-control" id="exampleInputEmail1" placeholder="Enter link"
